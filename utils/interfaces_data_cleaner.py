...
def load_irrelevant_phrases(file_path):
    """Load phrases to remove from an external file."""
    with open(file_path, 'r', encoding='utf-8') as f:
        return [line.strip() for line in f if line.strip()]


def remove_irrelevant_phrases(polish_source_file_path, kashubian_source_file_path, polish_destination_file_path,
                              kashubian_destination_file_pathsearch_phrases, search_phrases, search_in):
...
def clean_data():
    ...
    for file_name in file_names:
        source_dir = '../data/0_raw'
        destination_dir = '../data/1_cleaned'
        polish_file_source_path = f'{source_dir}/{file_name}.pl.txt'
        polish_file_destination_path = f'{destination_dir}/{file_name}.pl.txt'
